...
-(void) playNumberSound
{
    ...
    NSString *domStr = [NSString stringWithFormat:@"Number%02d.png",firstLegIdx];
    CCSprite *firstLegDom = [CCSprite spriteWithSpriteFrameName:domStr];
    NSInteger scndLegIdx = sumValue - firstLegIdx;
    domStr = [NSString stringWithFormat:@"Number%02d.png",scndLegIdx];
...
-(id) init
{
	...
	if( (self=[super init] )) {
		...
		[[CurrentLevel sharedCurrentLevel] setLevelNumber:[NSNumber numberWithInt:10]];
		[[CurrentLevel sharedCurrentLevel] setCurrentScore:[NSNumber numberWithInt:0]];
		// ask director the the window size
        
        [CCTexture2D PVRImagesHavePremultipliedAlpha:YES];
		CCSpriteBatchNode *levelBatch = loadBatchNode(self, @"level10Sprites");
		displayBackground (levelBatch,@"Level10BG.png");
